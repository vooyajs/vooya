...
const fixture = resolve(repositoryRoot, "tests/fixtures/rust-solid");

run("npm", ["run", "build", "--workspace", "@vooya/build-core"], repositoryRoot);
run("npm", ["run", "build", "--workspace", "@vooya/vite"], repositoryRoot);
run("npm", ["run", "build", "--workspace", "@vooya/solid"], repositoryRoot);
...
await verifyBrowser();
console.log("Verified Rust-file Solid component and store production behavior.");
...
function verifyGeneratedDeclarations() {
  ...
  assert.match(component, /import type \{ Component \} from "solid-js"/);
  assert.match(component, /onSelected\?: \(value: number\) => void/);
  assert.match(store, /import type \{ Accessor \} from "solid-js"/);
  assert.match(store, /state: Accessor<CartSnapshot \| undefined>/);
}
...
async function verifyBrowser() {
  ...
  try {
    ...
    if (errors.length > 0) throw new Error(`Rust-file Solid fixture had browser errors:\n${errors.join("\n")}`);
  } finally {
    await browser.close();
